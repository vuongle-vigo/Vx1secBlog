...
tags: ["malware development", "process injection", "APC", "shellcode", "windows security"]
description: "Techniques for injecting code using Asynchronous Procedure Call (APC) queue mechanism"
---
...
Use the `QueueUserAPC` API to queue an APC. For parameters and usage of this function, see Microsoft's documentation: [QueueUserAPC](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-queueuserapc).

After calling the above API, the APC function that was set up through QueueUserAPC is executed. The process must be placed in an alertable state. This can be achieved by creating a thread and then using one of the following APIs:
...
Additionally, there are 2 other ways:

- Create a process with the `CREATE_SUSPENDED` state and then use `ResumeThread`
- Create a process with the `DEBUG_PROCESS` state and then use `DebugActiveProcessStop` to continue the process.

### Code

The code is quite simple compared to other types of injection. Below is code that injects into a remote process to execute a payload that opens notepad.
The payload code was created as mentioned in another post of mine: [Payload Placement](https://github.com/vuongle-vigo/MalDevAcademy-Blog/tree/main/25-12-2023%20Payload%20Placement)
...
- VS2022: Build no debug info + MT release, detected as malware immediately after build completion, and transferred to virtual machine without needing to run (VirusTotal: 22/72)
- GCC: With faulty shellcode version, execution doesn't cause any issues (problem is in the shellcode)
- GCC: With standard shellcode version, scan not detected, execution also not detected on virtual machine, but still has potential for detection because shellcode is unpacked and has signature. (VirusTotal: 21/73)

=> Building with GCC can help improve code obfuscation.

### Update

- VS2022: After applying techniques from the Malware Compiling section, not detected if not using original shellcode. (VirusTotal: 22/72)
